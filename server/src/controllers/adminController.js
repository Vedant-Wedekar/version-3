...
// GET /api/admin/packages — admin sees ALL packages (including inactive)
export const listAllPackagesAdmin = asyncHandler(async (req, res) => {
...
// PATCH /api/admin/packages/:slug — toggle isActive / isFeatured
export const updatePackage = asyncHandler(async (req, res) => {
  const { slug } = req.params;
  const { isActive, isFeatured } = req.body;

  const ref = adminDb.collection("packages").doc(slug);
  const snap = await ref.get();
  ...
  await ref.update(update);
  const fresh = await ref.get();
  res.json({ success: true, package: { id: fresh.id, ...fresh.data() } });
});